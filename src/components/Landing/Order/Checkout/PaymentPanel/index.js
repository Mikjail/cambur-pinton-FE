...
export default class PaymentPanel extends Component {
  ...
  calcTotal() {
    ...
    let subtotal = amount + DELIVERY.firstRadius;
    let total = (subtotal * 0.9).toFixed(2);
    return total;
  }
  renderAlertAddress() {
    const { addressSelected } = this.props;
    if (!addressSelected) {
      return <div className="primary-link">Seleccione un Domicilio</div>;
    }
  }
  renderOnlinePayment() {
    ...
    if (!componentLoader) {
      return (
        <a
          href={paymentLink}
          className={
            "btn center " + (this.isAbleToPayOnline() ? "primary" : "disabled")
          }
        >
          Realizar Pago
        </a>
          ...
          <div className="card-body">
            <div className="card-panel card-panel-payment">
              <button
                ...
                onClick={() => this.toggleCardBody("cash-container")}
              >
                Efectivo
              </button>
              <div className="card-body" id="cash-container">
                ...
                <button
                  onClick={() => this.props.onSubmitOrder(history, this.props)}
                  className={
                    "btn center " +
                    (this.isAbleToPay() ? "primary" : "disabled")
                  }
                >
                  Pedir
                </button>
            ...
            <div className="card-panel card-panel-payment">
              <button
                className="card-title"
                onClick={() => this.toggleCardBody("pago-online")}
              >
                Pago Online
              </button>
