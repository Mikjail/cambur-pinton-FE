...
export default class AddressPanel extends Component {
    ...
    if (auth) {
      if (!checkedAddress && auth.addresses.length > 0) {
        ...
        let address = checkedAddress
          ? auth.addresses.find((address) => address._id == checkedAddress)
          : defaultAddress;
  ...
  renderAddresses() {
    ...
    if (!addresses) {
      return (
        <form onSubmit={this.props.handleSubmit(this.props.onSubmitAddress)}>
          {this.renderFields()}
              ...
              <li>
                <div className="address-text">
                  <input
                    type="radio"
                    value={address._id}
                    checked={checkedAddress === address._id}
                    onChange={this.handleCheck.bind(this)}
                  />
                  <span>
                    {address.telephone} - {address.street}, {address.floor}{" "}
                    {address.apartment}. {address.zone}{" "}
                  </span>
                </div>
    ...
    if (addresses) {
      return (
        <a
          ...
          onClick={() => {
            this.props.availableAddressStatus(false);
          }}
          className="right primary-link"
        >
          Agregar Domicilio
        </a>
      );
      ...
      if (auth.addresses.length > 0) {
        return (
          <a
            href="javascript:void(0);"
            onClick={() => {
              this.props.availableAddressStatus(true);
            }}
            className="right primary-link"
          >
            cancelar
  ...
  render() {
    return (
      <div className="card-panel">
        ...
        <div className="card-body">{this.renderAddresses()}</div>
      </div>
    );
  }
}
